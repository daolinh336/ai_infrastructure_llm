...
import { ClosedLoopGuard, ClosedLoopGuardError, loadClosedLoopGuardConfig } from '../agent/closed-loop-guard.js';
...
import type { PlannerRevisionRequest, RevisionHistoryRecord, RevisionObservation } from '../domain/types.js';
import { ExecutionEngine, type ApplyPreparationResult } from '../execution/execution-engine.js';
...
import { getStateDatabasePath, saveVerifiedRuntimeSnapshot } from '../state/sqlite-state-store.js';
...
program
  ...
  .action(async (prompt, options) => {
    ...
    const effectiveProvider =
      deployRequested && input.provider === 'stub' && process.env.OPENAI_API_KEY?.trim()
        ? 'openai'
        : input.provider;
    ...
    const agent = new ReActAgent(provider, reportProgress, {}, undefined, undefined, { logEnabled: true });
    ...
    if (result.status === 'clarification') {
      ...
      if (result.clarificationContext && result.uncertainties?.length) {
        console.log();
        console.log(chalk.cyan('Answer now to let planner continue in same run.'));
        ...
        if (resumedResult.status !== 'planned') {
          ...
          if (resumedResult.status === 'clarification') {
            console.log(chalk.yellow('Still needs clarification after first answer.'));
        ...
        console.log();
        console.log(chalk.green('Clarification applied. Continuing with resumed plan.'));
    ...
    if (execution.secretResolution && execution.secretResolution.services.some((svc) => svc.secrets.some((sec) => sec.source === 'auto-generated'))) {
      console.log(chalk.gray('💡 Auto-generated passwords are saved to: state/generated-secrets.env (after --apply).'));
      console.log(chalk.gray('💡 .env only seeds passwords for NEW services; deployed services keep passwords in state/Docker.'));
      console.log();
    }
    ...
    if (applyRequested) {
      ...
      let { approval, decision: previewDecision } = await requestCliApproval(applyPreparation.approvalRequest);

      if (previewDecision.choice === 'other') {
        console.log(chalk.cyan('User provided feedback. Returning to planner for revision...'));
        ...
        console.log(chalk.cyan(`- Revision summary: ${revisionResult.revisionSummary}`));
        ...
        const revisedResult = {
          ...
          plan: {
            ...
            assumptions: [...result.plan.assumptions, ...revisionResult.assumptions],
          },
          observations: [
            ...result.observations,
            {
              source: 'observe:user_feedback',
              message: previewDecision.userFeedback?.message ?? 'User selected other without additional feedback.',
            },
            {
        ...
        applyPreparation = await engine.prepareApply(revisedResult);
        printPreflightReport(applyPreparation.preflight);
        if (!applyPreparation.approvalRequest) {
          console.log(chalk.red('Phase 8 apply stopped after feedback revision. Preflight failed; docker-compose.yaml was not written.'));
          process.exitCode = 1;
          return;
        }
        console.log(chalk.cyan('Revised docker-compose.yaml preview:'));
        console.log(applyPreparation.composeYaml);
        ({ approval, decision: previewDecision } = await requestCliApproval(applyPreparation.approvalRequest));
      }
      ...
      if (applyResult.generatedSecretsPath) {
        ...
        console.log(chalk.gray('  💡 .env only seeds new services; deployed services keep passwords in state/Docker.'));
      }

      if (deployRequested) {
        console.log(chalk.cyan('Deploying to Docker via MCP...'));
        const mcpClient = new DockerMcpGateway();
        const closedLoopGuard = new ClosedLoopGuard(loadClosedLoopGuardConfig());
        try {
          ...
          if (deployLoopResult.status === 'passed') {
            ...
            console.log(chalk.red(`Closed-loop deploy ended with status: ${deployLoopResult.status}`));
            process.exitCode = 1;
          }
        } catch (error) {
