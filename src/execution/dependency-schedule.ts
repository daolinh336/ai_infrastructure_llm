...
export function evaluateDryRunPolicy(
  ...
  for (const service of validSpec.services) {
    ...
    for (const [key, value] of Object.entries(service.environment ?? {})) {
      ...
      if (isAutoGeneratedSecret(value)) {
        findings.push({
          severity: 'info',
          code: 'auto-generated-secret',
          message: `Service "${service.name}" uses an auto-generated secret for ${key}; see state/generated-secrets.env after apply.`,
          resourceName: service.name,
          resourceType: 'service',
        });
      } else if (isWeakHardcodedSecret(value)) {
        findings.push({
          severity: 'warning',
...
function isWeakHardcodedSecret(value: string): boolean {
